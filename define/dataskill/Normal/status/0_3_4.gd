...
var skill_name := "魔法增效"        # 技能名

# 实现脚本
var impl_class = "res://define/dataskill/Normal/status/cs/DuckSkill0_3_4.cs"
...
# ---- 图标 ----
var icon_path := "res://IMG/skill/Normal/status/0_3_4.png"    # 技能图标图片地址
...
# ---- 增减益 ----
var gain_energy := 5                # 获得能量
var gain_hp := 0                    # 获得血量（默认0，正数=获得，负数=扣除）
var gain_buff := [
	{"stat": 3, "layer": 7, "value": 10, "is_ratio": true, "active_mode": 5},
]
# 获得 Buff：stat=3=MATK魔攻；layer=6层；value=10每层10%；is_ratio=true百分比；active_mode=6永久 → 魔攻+60%
var gain_buff_bingo := [
...
# ---- 描述 ----
var main_description := ["凝聚魔力，大幅提升魔法攻击力"]                    # 主描述（数组类型）
var auxiliary_description := ["状态技能，使用时自身不进行攻击，魔攻+70%"]        # 辅助描述（数组类型）