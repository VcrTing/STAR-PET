...
[GlobalClass]
public partial class DuckSkill0_3_5 : Resource
{
    public void DoSkill(int index, FightRunning run, InsFightSkill sideSkill)
    {
        // 连续热身效果：根据 BingoSkillType 判断使用 GainPower 还是 GainPowerBingo
        Godot.Collections.Array powerSource = run.BingoSkillType == EnumSkillType.DEFENSE
            ? sideSkill.Skill.GainPowerBingo
            : sideSkill.Skill.GainPower;
        FightSkillStatusRunTool.ExecuteStatusPower(run, index, powerSource);
    }
