...
/// <summary>
/// 敌方战斗中能力（Power）管理器
/// 与 FightYouStandBuffManager 结构一致，管理"能力"而非"个体值 Buff"
/// </summary>
public partial class FightYouStandPowerManager : Node2D
{
    private static FightYouStandPowerManager _instance;
    public static FightYouStandPowerManager Instance => _instance;
    ...
    public void AddPowers(InsFightPower[] newPowers)
    {
        ...
        GD.Print($"[FightYouStandPowerManager] AddPowers: {newPowers.Length} 个能力加入");
        ...
        foreach (var power in newPowers)
        {
            ...
            if (existing != null)
            {
                ...
                GD.Print($"      [FightYouStandPowerManager] 同类能力叠加: PowerType={power.PowerType} Layer={existing.Layer} Value={existing.Value}");
            }
            else
            {
                // 不同类能力：直接添加
    ...
    public void WhenPetDisAppear(InsFightPetData pet)
    {
        ...
        if (removed > 0)
        {
            GD.Print($"[FightYouStandPowerManager] 精灵 [{pet.PetName}] 离场，清除非永久能力 {removed} 个");
