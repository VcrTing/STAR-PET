...
public partial class FightMyStandBuffManager : Node2D
{
    ...
    public void AddBuffs(InsFightBuff[] newBuffs)
    {
        ...
        foreach (var buff in newBuffs)
        {
            ...
            if (existing != null)
            {
                ...
                GD.Print($"      [FightMyStandBuffManager] 同类 Buff 叠加: Stat={buff.Stat} Layer={existing.Layer} Value={existing.Value}");
            }
            else
            {
                // 不同类 Buff：直接添加
    ...
    public void WhenPetAppear(InsFightPetData pet)
    {
        ...
        if (VBoxViewBuffsContentMy.Instance != null)
        {
            ...
            VBoxViewBuffsContentMy.Instance.UpdateBuffs(buffs);
        }
    }
    ...
    public void WhenPetDisAppear(InsFightPetData pet)
    {
        ...
        if (removed > 0)
        {
            GD.Print($"[FightMyStandBuffManager] 精灵 [{pet.PetName}] 离场，清除非永久 Buff {removed} 个");
